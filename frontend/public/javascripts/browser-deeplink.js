...
(function (root, factory) {
    ...
    var getWebLink = function () {
        var linkmap = {
            "ios": settings.iOS.fallbackWebUrl || window.location.href,
            "android": settings.android.fallbackWebUrl || window.location.href
        };
    ...
    var open = function (uri) {
        if (!isMobile()) {
            alert("Please open this url in mobile");
            return false;
        }

        if (isAndroid() && settings.androidDisabled) {
            alert("android setting disable");
            return;
        }
