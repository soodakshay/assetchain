...
var chaincodeName = config.chaincode;
...
module.exports.prepareInvokation = function(functionName, args, ch){
    ...
    return new Promise((resolve, reject) =>{
        ...
        if(!channel.status==200){
            reject(createErrorResponse("channel couldnot be retrieved", httpStatus))
        }
        ...
        userContext(channel).then((obj) =>{
            ...
            if(proposalResponse
                ...
                var eventHubAddress = (config.network_type == 0)
                                        ? 'grpc://localhost:7053' 
                                        : 'grpc://peer2.debutinfotech.com:7053';
...
function emitBlockInfo(){
    if(global.socketChannel=="debutchannel")
    ...
    network.chaininfo(global.socketChannel).then(function(queRes){
       ...
       reject("Error while fetching block height ==> " + err)
    });
}
...
function getUserChannel(channelName){
    ...
    try{
        var channel = fabric_client.getChannel(channelName)
        peer0 = channel._peers[0]
    } catch(e){
        console.log(e.message)
        console.log(e.__proto__.name)

        if(e.__proto__.name=="Error" || e.message == "Channel not found for name "+channelName+"."){
            ...
            if(config.network_type == 0){
                ...
                var order1 = fabric_client.newOrderer('grpc://orderer0.debutinfotech.com:7050')
